...
router.get('/manage/user/list', (req, res) => {
    UserModel.find({username: {'$ne': 'admin'}}).populate('role_id')
        .then(users => {
...
router.get('/manage/product/list', (req, res) => {
    ...
    ProductModel.find({}).populate('categoryId').populate('pCategoryId')
        .exec()
        .then(products => {
...
router.get('/manage/product/search', (req, res) => {
    ...
    ProductModel.find(contition).populate('categoryId').populate('pCategoryId')
        .exec()
        .then(products => {
            res.send({status: 0, data: pageFilter(products, pageNum, pageSize)})
